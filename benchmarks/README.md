...
| ------ | ------------- | ----------------: | ------------------------: | ------------------------: |
| flat   | bean → record |             4.399 |                     5.437 |                    340.13 |
| flat   | record → bean |             4.091 |                     5.791 |                    464.27 |
| nested | bean → record |             5.638 |                    10.445 |                    501.43 |
| nested | record → bean |             5.735 |                    11.648 |                    709.86 |
| deep   | bean → record |            50.509 |                    59.482 |                   2024.06 |
| deep   | record → bean |            63.082 |                    64.168 |                   2292.43 |
...
~1.0 ns. On nested (5.64 vs 10.45 ns), 1.85× behind, ~4.8 ns absolute. On deep (50.51 vs 59.48 ns), 1.18× behind on
forward, parity on backward (63.08 vs 64.17).
...
that hop still costs ~1 ns of constant overhead. On flat and nested the actual work is only 4–11 ns, so the overhead
