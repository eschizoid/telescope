...
**MapStruct wins on every row.** No spin. The closest comparison — codegen vs codegen — has MapStruct ahead by **2–4×**
across the three tiers. The runtime path is **30–100× slower** than MapStruct's generated bytecode. Both are still
sub-microsecond for flat/nested and just past one microsecond for the deep tier.

**Why MapStruct is faster on this benchmark:**

- MapStruct emits one hand-templated method body per pair. Every getter / setter / nested-mapping call is fully
  monomorphic and the JIT inlines the whole conversion into a single basic block. No composition, no dispatch
  indirection.
- Telescope's `@Bridge` codegen emits an `Iso` chain composed via the lattice's `.then(...)` rules. The composition is
  what makes the lattice reusable (an `Iso` round-trips, threads through `Telescope` paths, lifts through containers via
  `Iso.liftList`) — but at the dispatch level, each `.then` is a virtual call the JIT has to inline. On a tight bench
  loop it usually does; on cold paths or larger workloads it sometimes doesn't, and you pay the indirection.
- Telescope's runtime path is reflective-with-LMF-dispatch. Even after ADR-0005's LMF substrate (single-step LMF
  dispatch is in the same band as `Method.invoke` per the LmfBenchmark section above), the composed lens chain has to
  walk the full record / bean spine at every level, which is the dominant cost.

**What this means for picking a tool:**

- **Pure dispatch speed: MapStruct.** If your problem is "convert this entity to this DTO and back, both directions are
  known at build time, no other capability needed," MapStruct's generated bytecode is the right answer.
- **Asymmetric capabilities: telescope.** The sealed-narrow paradigm hop, effectful update (`updateAsync` /
  `updateValidated`), JPA cycle handling, Hibernate LAZY proxy unwrap, deep navigation as a primitive — all are out of
  scope for MapStruct's mapping-only model and demoed end-to-end in the `examples/springboot/` modules. None of those
  are perf wins; they're capability wins.
- **Sub-microsecond context.** Even the slowest telescope row (deep runtime backward, ~2.7 μs) is fine for typical
  request-handling code — REST → JPA → response sees one or a few conversions per request, not millions per second. The
  2–4× codegen gap (~10–140 ns absolute) is irrelevant for service workloads; it matters when conversion is the hot loop
  in a batch pipeline.

**Caveats:**

- 5 measurement × 1 fork is the project's standard JMH config; it's not a publication-grade run. The
  `deep_mapstruct_backward` ±68.75 ns error bar is wider than the score itself — the backward direction on the deep tier
  was noisy this run. A 10 measurement × 3 fork run would tighten it.
- The Telescope `_codegen_backward` rows use `BRIDGE.set(placeholderBean, rec)` which discards the placeholder and
  invokes the underlying `Iso.from(rec)`. The `set` adds a small constant overhead vs a direct `iso.from` call; on flat
  the codegen backward (6.85) is actually faster than forward (15.37), which suggests the JIT is handling the `set` call
  site well in this case.
