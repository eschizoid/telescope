...
Runtime conversion (`Telescope.from/to/using`) runs ~24–60× slower than MapStruct's generated bytecode after the PR #118
Object[] intermediate change. The reflective lens chain walks the record/bean spine at every level (cached LMF readers,
not raw reflection), and per-call allocation is now 64–384 B/op (down from 776–1296 B/op). Sub-microsecond on flat and
nested, single-microsecond on deep. Reach for codegen on hot paths; the runtime path is for one-shot conversions and
non-hot service code.
