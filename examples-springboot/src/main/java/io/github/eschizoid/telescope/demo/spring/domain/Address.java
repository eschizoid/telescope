 ...
 *
 * <p>{@link Focus @Focus} triggers {@code FocusProcessor} to emit {@code AddressPath<R>} +
 * {@code AddressTelescope}. The shared {@code OrderMappers} config handles the cross-paradigm
 * conversion between {@code Address} and {@code AddressEmbeddable} via {@code Telescope.mapper(...)};
 * the generated path navigator powers compile-time-typed deep updates in the codegen controller.
 *
 ...
 * packages (here {@code domain} vs {@code persistence}). Filing the bug as a follow-up; for now
 * the runtime mapper covers the conversion just fine.
 */
@Focus
