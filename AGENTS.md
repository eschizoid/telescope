...
The user-facing tour lives in `README.md`. This file is for _implementers_. Conventions in it are enforced by review,
not by the compiler, so read the mantras before the module map.

---

## Project mantras (must-honour on every PR)

1. **No inline FQN names in code.** Always add an `import` and use the simple name. The repo has been swept clean of
   inline FQNs (PR #108); reintroducing them is a regression. Includes `java.util.LinkedHashMap` in method bodies,
   `javax.lang.model.SourceVersion` in processors, etc. — _add the import_.
...
7. **A test that cannot fail is not a test.** Before claiming a test guards a change, revert the change and watch it go
   red. Several tests in this repository's history asserted a property that held both before and after the fix — a
   container rebuild whose size and contents are identical either way, a filtered read whose match sat at the last
   element where stopping and walking cost the same. Pick the input where the behaviours diverge.

## Writing conventions

- **Comments state a property, not the incident that taught it.** A good comment names something true about the world
  ("a table sized for n entries resizes once n passes three quarters of the next power of two"). A bad one names an
  event in this repository's history ("the review caught this", "fixed in round 2", "this used to be slow"). The
  incident belongs in the PR description; the property belongs in the code.
- **Never name external organisations** in commits, PR bodies, or comments. Adopter feedback drives priority without
  being cited by name.
- **Prose in Markdown is one line per paragraph.** GitHub renders a single newline as a line break, so hard-wrapped
  prose reads as ragged. Fixed-width blocks (code fences, tables) keep their formatting. Commit messages are the
  opposite: wrap at ~72 columns, since they are read in terminals.
...
- **Public** — `io.github.eschizoid.telescope.*` — what users name. One DSL class (`Telescope<S, A>`), three
  annotations, two sealed effect types, one Indexed record.
...
**plus a `Function<S, S> chain`** (defaults to identity) that accumulates multi-edit work. The whole user-visible
...
- **Write — multi-edit (recommended):** `Telescope.all(Edit<S>...)` folds N edits into one reusable normalizer.
  ...
  `.with(...)` uses. Sequential (one structural pass per edit) in v1; fusion across shared prefixes is the open
  extension via an internal `Traversal#structure()` method without touching public surface.
...
- **Convert — declarative mapping (recommended):** `Telescope.map(Mapping<A, B>...)` — varargs sibling of fluent
  `Telescope.map(A.class).to(B.class).field(...).build()`. Each row is a `Mapping` static factory: `to(srcAcc, tgtAcc)`
  same-typed; `to(srcAcc, tgtAcc, fwd, bwd)` typed transform; `via(srcAcc, tgtAcc, mapper)` nested; `auto()` same-name
  backfill (sibling inference); `auto(A.class, B.class)` same-name with explicit classes. Sibling
  `Telescope.mapper(...)` returns `Mapper<A, B>` for patch/nestability. Symmetrical with `Telescope.all(Edit<S>...)`.
  The only runtime-checked corner: all-bare-`auto()` rows have no source/target classes to infer — throws a precise IAE
  pointing to `auto(A.class, B.class)`.
...
Each optic exposes `.then(Other)` for composition. Composition picks the most-specific result type; the lattice's
resolution rules are in the composition table further down. The lattice is what makes the public DSL composable without
users seeing optic types.
...
- **Entry points**: `Telescope.of(Class)` / `ofBean(Class)`, `lens(getter, setter)`,
  `from(Class).to(Class).using(fwd, bwd)`, `map(A).to(B)…build()` / `mapBean(A).to(B)…build()`,
  `fromBean(P).to(R).via*()`.
...
- **Read**: `read` / `find` / `toList` / `toListIndexed` / `count` / `exists`.
- **Bean-bridge** `.rename(Accessor, Accessor)`: both accessors bound to a shared `<X>` property type.
...
- `.each()` no-arg — runtime dispatch on the current focus's container shape. Used when you already hold a
  `Telescope<S, SomeContainer>` from a pre-built path. `javac` can't enforce `A extends Iterable<?>` at method level.
...
| File                                                      | Role                                                                                                                                                                                                                                                                                                 |
| --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AbstractTelescopeProcessor.java`                         | **public abstract** base. Holds the shared emit pipeline (`emitBeanNavigator`), the `javax.lang.model` probes (setter/builder discovery, traversal-shape detection, navigable-type lookup), and the forwarder block. Subclassed by `BeanFocusProcessor` and (out of tree) by `LombokFocusProcessor`. |
...
| `META-INF/services/javax.annotation.processing.Processor` | SPI registration for all three.                                                                                                                                                                                                                                                                      |
...
The pattern (followed by `LombokFocusProcessor` and `BridgeProcessor`): keep an instance-field `pending` set, collect
Lombok-annotated targets every round, only emit on `roundEnv.processingOver()` when Lombok is guaranteed done patching.
**Any future telescope processor reading synthesised members of Lombok-annotated types must follow this pattern.**
...
| `TelescopeConfig`         | `TelescopeProperties`        | Single `telescope.registry.fail-fast` toggle (default `true`).                                  |
...
skip Quarkus's startup bytecode scan. Strict `module-info.java` requires `--add-reads ALL-UNNAMED` (quarkus-arc and
smallrye-config ship no JPMS info). Runtime-only — no separate `deployment` module / `@BuildStep` recorders.

**Spring specifics.** `TelescopeAutoConfiguration` is `@AutoConfiguration` registered via
`META-INF/spring/...AutoConfiguration.imports`. Injects `List<Mapper<?, ?>>`; exposes the registry as
`@Bean @ConditionalOnMissingBean`. `TelescopeProperties` is `@ConfigurationProperties("telescope.registry")` with the
same `failFast` flag.
...
Plain JUnit 5. Cover the DSL surface (`TelescopeTest`, `CompanyDemo`), the optic laws (`OpticLawsTest`), the indexed
traversal (`IndexedTraversalTest`), the effectful update variants, the from/to/using factory, and the bean navigation
path. No special harness.
...
takes a JMH filter, iteration counts, fork count, and an optional profiler. A developer machine running anything else in
the background produces numbers that look like findings and are not.
...
| `NavigatorBuildBenchmark`                                  | generated-navigator path construction, against a prebuilt floor                    |
| `IntoBenchmark`                                            | `Mapper.into` against `forward` at two arities                                     |
| `LmfBenchmark`                                             | the dispatch substrate: LMF vs reflection vs hand-rolled                           |
| `EffectfulTraversalBenchmark`                              | the `update*` effect variants against a pure update                                |
...
Every performance defect found in the September 2026 audit lived on a path no benchmark exercised — reads on a
navigation path, name resolution on a wide record, hash-container writes, in-place `into`. The benchmarked paths were
fine. When adding a benchmark, prefer a dimension nothing else varies (cardinality, arity, path depth) over another
variant of a shape already covered.
...
rewrites. Tested at both layers: `OpticLawsTest` proves the optic laws, `TelescopeTest` + `CompanyDemo` prove DSL
behaviour.

The history (function registry → full Monocle port → throw-away-the-lattice regression → settle on two layers) is in a
series of posts on the author's blog.
...
field name. The error message tells you. For dynamic field names there's a `.field(String)` overload.
...
Plain `RecordComponent.getAccessor()` and `Lookup.unreflect` — no MethodHandles indirection. ~100ns per operation vs
~5ns for hand-written record copies. Fine for almost everything; codegen exists for hot paths. See
`docs/adr/0003-reflection-over-method-handles.md`.
...
native binary (8 capabilities) is the regression gate — on every substrate push to `main` and weekly. Full contract:
...
5. **0005 — LambdaMetafactory over MethodHandle.invoke.** Refines ADR-0003: keep reflective discovery; swap hot-path
   ...
   Phased: records (#9), bean getters (#12), bean setters (#11), builder writers (#13), rebuild path (#14).
...
15. **0015 — Native-image AOT support for the runtime path.** (Accepted, shipped #250; qualifies ADR-0005 under AOT.)
    ...
    `:examples:graphql` `NativeVerify` binary (8 capabilities) is the CI regression gate.
...
lives in the issue tracker — the performance audit is tracked under one umbrella issue with per-defect children, each
carrying its mechanism, a measurement, and a proposed verification.

Good first contributions tend to be the ones with a mechanism already written down and a benchmark already in place. If
you are picking one up, the two things reviewers will ask for are the ones the mantras name: a measurement with a
...
- **A hash container's `int` constructor takes a table capacity, not an element count.** `new HashSet<>(n)` filled with
  `n` elements resizes once `n` passes three quarters of the next power of two. Use `HashSet.newHashSet(n)` and its
  siblings. `ArrayList(n)` _is_ an exact element capacity; `ConcurrentHashMap` and `IdentityHashMap` already size for
  the count internally. Do not "fix" those two.
- **Generated code must read each source property exactly once.** A conversion expression that names its read twice
  ...
  returns non-null then null. Hoist into a local; the emitters have a prelude idiom for this.
