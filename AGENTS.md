...
| `annotations/`               | `@Focus` (records → navigator), `@BeanFocus` (POJOs), `@Bridge`/`@Bridges` (type-pair conversion), `@FromMap` (untyped ingestion), plus the per-field codegen knobs (`@Compute`, `@Constant`, `@Default`, `@Rename`, `@Transform`, `@ViaMapper`, `@UncheckedMapping`; `WriteStrategy` is an enum passed to `@Bridge`, not an annotation) |
