package org.telescope.internal.optics.instances;

import java.util.ArrayList;
import java.util.List;
import java.util.function.BiFunction;
import java.util.function.Function;
import org.telescope.Validated;
import org.telescope.internal.optics.Applicative;
import org.telescope.internal.optics.Kind;

/**
 * {@link org.telescope.internal.optics.Kind.Witness} + {@link
 * org.telescope.internal.optics.Applicative} instance for {@link Validated} fixed to a specific
 * error type {@code E} — the effect lifted by {@link
 * org.telescope.internal.optics.Traversal#modifyF}. Semantics: accumulate errors across both
 * arguments of {@code map2} (concatenate the two {@code Invalid} error lists). {@code pure} is a
 * {@code Valid}.
 *
 * <p>This accumulating {@code map2} is the whole reason {@code Validated} exists separately from
 * {@link org.telescope.Either} — when one branch fails, we still want to evaluate the other and
 * report every problem, not stop at the first. Accordingly there is no {@code isFailed} override
 * (it stays {@code false}), so {@code modifyF} processes every focused element rather than
 * short-circuiting.
 *
 * <p>Used by {@link org.telescope.Telescope#updateValidated}.
 */
