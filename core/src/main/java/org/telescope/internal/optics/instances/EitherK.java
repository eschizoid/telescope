 ...
 * org.telescope.internal.optics.Applicative} instance for {@link Either} fixed to a specific left
 * type {@code L} — the effect lifted by {@link org.telescope.internal.optics.Traversal#modifyF}.
 * Semantics: short-circuit on the first {@code Left}. {@code pure} is a {@code Right}, {@code map2}
 * returns the first {@code Left} it sees, and {@code isFailed} returns {@code true} for a {@code
 * Left} — so {@code modifyF} stops invoking the per-element function once a failure is reached.
 *
 ...
 *
 * <p>Used by {@link org.telescope.Telescope#updateEither}.
 */
