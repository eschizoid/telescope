...
`observeAsync(callback, executor, onError)` reuses `observe` semantics and submits one task per reached value. It does
not create a future per callback or wait for callbacks. Reads submit current values; writes submit rebuilt values after
deeper edits. A callback is a synchronous Consumer: if it starts another async operation, it must handle that
operation's eventual failure itself.

Use an application-owned executor with a bounded queue, for example:

```java
final var callbackExecutor = new ThreadPoolExecutor(
  2,
  2,
  0L,
  TimeUnit.MILLISECONDS,
  new ArrayBlockingQueue<>(1024),
  Thread.ofPlatform().name("telescope-observer-", 0).factory(),
  new ThreadPoolExecutor.AbortPolicy()
);
```

Import these types from `java.util.concurrent`. These sizes are examples, not library defaults. Close this executor at
application shutdown. Closing it around every transformation waits for tasks and defeats deferred execution.
`AbortPolicy` lets Telescope report rejection through `onError` and continue. Silent-discard policies cannot report lost
work; caller-runs policies and direct executors can execute callbacks inline and block transformation. The error handler
runs on the callback thread for callback failures, or the submitting thread for rejection, so keep rejection handling
fast. A runtime exception from the error handler is logged; Errors are not caught.

For remote metrics, observe Teams to submit fewer tasks than observing every User:
...
final var emails = Telescope.of(Company.class)
  ...
  .observeAsync(
    (team) -> metricsClient.publishTeamSize(team.name(), team.users().size()),
    callbackExecutor,
    (team, error) -> log.warn("Team metrics export failed for {}", team.name(), error)
  )
  .each(Team::users)
  .field(User::email);
```

`metricsClient` is an application-provided service whose synchronous send runs on the executor. Use synchronous
`observe` for cheap local counter increments. Callbacks may complete in any order with multiple workers; one worker
preserves accepted task order, although concurrent transformations can interleave submissions.

The [README message queue example](../README.md#with-telescope) follows the same pattern: publish a rebuilt User
snapshot on the executor, then let the transformation continue. Use it for best-effort downstream work such as
refreshing a search index. `observeAsync` does not coordinate with a database transaction or wait for broker
acknowledgement. If the publisher itself returns a future, attach a failure handler to that future inside the callback;
`onError` only sees synchronous callback failures and rejected task submissions. Use a transactional outbox when
publishing must be coupled to a database commit.

Values are passed by reference without copying, and callbacks must not mutate traversed data. Arrange immutable
snapshots if other code can mutate the observed objects. ThreadLocal/MDC context is not automatically propagated.
Callbacks run on reads too; this is not a transaction-commit hook. Failed input futures and typed effect failures skip
reconstruction and observation, but later reconstruction failures cannot retract already submitted work. Shutdown may
discard queued tasks. `trace` and `explain` never submit callbacks.
