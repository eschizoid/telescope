...
annotations, zero build step — now converts within **~1.3–4× of MapStruct** (flat ~4×, nested ~2×, deep ~1.3–1.9×), down
from ~16–48× before the conversion leaf was rebuilt. When a loop turns hot, `@Focus` / `@Bridge` codegen puts you in the
**same performance class** as MapStruct's generated bytecode. It also runs under **GraalVM native-image** — runtime path
included ([verified by a native binary in CI](docs/native-image.md)), a combination codegen-only MapStruct can't match.
Spring Boot starter and Quarkus extension ship as separate artifacts.
