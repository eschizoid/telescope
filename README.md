...
**The modern MapStruct alternative.** You map fields with **typed method references** — `Order::customerName`, checked
by `javac` and moved by your IDE's rename refactor. MapStruct's `@Mapping(target = "fullName", source = "customerName")`
binds by **string name, not by reference**: a rename never touches it, so it drifts out of sync — and under the default
`unmappedTargetPolicy` you get a build _warning_, not an error, with a `null` at runtime. The method reference just
moves with the field. That is the whole argument, and it is why you switch. Then a strictly larger surface — build a
typed path through your nested data and read, update, or convert through it, in one line — plus mappers that explain
themselves and a reverse direction that comes free.
...
Works on Java records, POJOs, and Lombok `@Data` classes, on Java 21+. And it is fast: the **runtime** path — zero
annotations, zero build step — now converts within **~1.3–4× of MapStruct** (flat ~4×, nested ~2×, deep ~1.3–1.9×). When
a loop turns hot, `@Focus` / `@Bridge` codegen puts you in the **same performance class** as MapStruct's generated
bytecode. It also runs under **GraalVM native-image** — runtime path included
([verified by a native binary in CI](docs/native-image.md)), a combination codegen-only MapStruct can't match. Spring
Boot starter and Quarkus extension ship as separate artifacts.
...
duality. The `@Mapping` string binds by name, not by reference: a rename drifts it out of sync — a build warning under
the default policy, a `null` at runtime — while `Order::customerName` is checked by `javac` and moves with the IDE
refactor. That is the argument. On the band they share (deep record↔record / bean↔record / bean↔bean), telescope's
`@Focus` / `@Bridge` codegen matches MapStruct's generated bytecode, and the annotation-free **runtime** path is now
within ~1.3–4× of it — no build step, competitive enough for the sub-microsecond conversions service code actually does.
Beyond that band, telescope keeps going where MapStruct's architecture stops. Two questions decide it — is it as fast,
and what do you gain — in that order.
...
> build step, and now **within ~1.3–4× of MapStruct** (flat ~12 ns / ~4×, nested ~15 ns / ~2×, deep ~90 ns / ~1.3–1.9×),
> down from ~16–48× before the leaf was rebuilt. It's closest exactly where real trees are deepest: on container-heavy
> nesting a `List`/`Set`/`Map` element loops over the leaf's raw handle, and a nested record/bean field inlines straight
> into the parent's handle — no per-nesting dispatch. Fast enough for most service code as-is; on a tight inner loop,
> add `@Bridge` and you're back in MapStruct's performance class. The runtime-vs-codegen numbers are in
> [`benchmarks/README.md`](benchmarks/README.md#mapstruct-comparison-apples-to-apples).

#### Native-image — the runtime path survives AOT

Codegen was never the question: MapStruct's and telescope's generated mappers are both reflection-free and build under
...
| GraalVM native-image               | codegen zero-config, and the runtime path survives AOT too            | codegen only — no runtime path      |
...
- You're navigating a mix of **records and POJOs** at any depth and don't want to materialize intermediate DTOs to
  bridge between them
- You're deploying to **GraalVM native-image** and want mapping without a build step — telescope's runtime mapper
  survives AOT (verified by an eight-capability native binary in CI, [`docs/native-image.md`](docs/native-image.md));
  with MapStruct, codegen is the only option
...
`Telescope.all(over(...), over(...))` runs **one structural pass per edit** — each `over(...)` navigates from the root
and rebuilds the spine. That is the right default: each edit stays on its own line and the paths can be completely
unrelated. But when several edits target fields **under the same parent**, you can collapse them into a single pass with
no extra API: navigate to the common ancestor and do the multi-field work in one `update` lambda.
