...
**The modern MapStruct alternative.** You map fields with typed method references: `Order::customerName`, checked by
`javac` and moved by your IDE's rename refactor. MapStruct's `@Mapping(target = "fullName", source = "customerName")`
binds by string name. A rename never touches the string, so it drifts out of sync, and under the default
`unmappedTargetPolicy` that costs you a build _warning_ and a `null` at runtime. The method reference just moves with
the field. That's the whole argument, and it's why you switch. Then you get a strictly larger surface: build a typed
path through your nested data and read, update, or convert through it in one line, plus mappers that explain themselves
and a reverse direction that comes free.
...
Works on Java records, POJOs, and Lombok `@Data` classes, on Java 21+. It's fast, too: the runtime path, with zero
annotations and no build step, converts within **~1.3–4× of MapStruct** (flat ~4×, nested ~2×, deep ~1.3–1.9×). When a
loop turns hot, `@Focus` / `@Bridge` codegen puts you in the same performance class as MapStruct's generated bytecode.
And it runs under GraalVM native-image, runtime path included
...
story, including `trace(input)` with real values and narrating every conversion by flipping a log level, is under
[Introspection](#introspection--see-what-a-mapper-does-explain--trace).
...
Same job, newer architecture. MapStruct is a mapping framework: string-keyed `@Mapping` annotations, compile-time-only
generation, one direction per interface, purpose-built for flat `Entity → Dto` conversion. Telescope is an optics DSL
where that same mapping is one capability among navigation, deep updates, effectful update, and sealed-type narrowing,
built on typed method references, records, sealed types, and a runtime-or-codegen duality. The `@Mapping` string binds
by name; a rename drifts it out of sync (a build warning under the default policy, a `null` at runtime) while
`Order::customerName` is checked by `javac` and moves with the IDE refactor. On the band they share (deep record↔record
/ bean↔record / bean↔bean), telescope's `@Focus` / `@Bridge` codegen matches MapStruct's generated bytecode, and the
annotation-free runtime path is now within ~1.3–4× of it: no build step, competitive enough for the sub-microsecond
conversions service code actually does. Beyond that band, telescope keeps going where MapStruct's architecture stops.
Two questions, in order: is it as fast, and what do you gain.
...
constructor and accessor calls the JIT inlines into one tight basic block. On the shape real services run (deeply nested
records with lists inside) they are a tie: ~1.15×, about 6–7 ns on a ~50 ns conversion, stable across repeated CI runs.
On a trivial flat 5-field struct the gap is ~1.07×, well under a nanosecond. At every one of these scales you are
...
The full CI-reproducible matrix (both directions, all three tiers, the runtime path, the methodology, the
...
branch, tune the iteration and fork knobs, and the run prints `results.txt` and attaches the full results as an
artifact.
...
> into the parent's handle, with no per-nesting dispatch. Fast enough for most service code as-is; on a tight inner
> loop, add `@Bridge` and you're back in MapStruct's performance class. The runtime-vs-codegen numbers are in
...
Codegen was never the question. MapStruct's generated mappers are reflection-free, telescope's are too, and both build
under GraalVM native-image without config. What MapStruct doesn't have is a runtime path, so under native-image its
story ends there. Telescope's runtime mapper keeps working inside a native image: `Telescope.mapper(...)` and
`.field(User::name)`, zero annotations, no build step. The trick is small. `LambdaMetafactory` defines a class at run
time, which AOT forbids, so inside an image the substrate builds its accessors as plain `MethodHandle` closures instead.
One `static final boolean` picks the branch, and the JVM hot path never changes.

You configure nothing for telescope itself; `telescope-core` carries its own native-image metadata in the jar. You
register your own types, same as any GraalVM app: DTO classes in `reflect-config.json`, plus the call-site class in
`serialization-config.json` if you navigate with method references. `.fieldByName(String)` skips that second file, and
codegen needs no config at all.

So what do you actually get? Fast startup and small images. Codegen mappers keep their JVM speed. The runtime path runs
a bit _slower_ in-image (`MethodHandle.invokeExact` instead of the JIT-warmed `LambdaMetafactory`), so nobody should
reach for native-image to speed up reflective mapping. None of this is a roadmap slide, either: an eight-capability
verifier covering the runtime mappers over records, beans, and builders, method-reference navigation and reads,
`@FromMap`, and `@Bridge` compiles and runs as a real native binary in CI on every substrate push to `main`, plus
weekly. The adopter contract is in [`docs/native-image.md`](docs/native-image.md).

#### Then, what you gain

Nanoseconds aren't the decision; scope is. Most of the comparison reads "telescope: yes / MapStruct: not in scope":
...
The full accounting is the [parity matrix](docs/mapstruct-parity.md): all 29 MapStruct features, each scored with the
telescope idiom, its honest limitation, and `file:line` evidence. One source of truth, kept next to the code it cites.
...
The intent is identical. The trade is not: MapStruct gives up typed refs for string-path brevity, and telescope gives up
the brevity for the guarantee that everything you wrote against the source and target types compiles iff it still makes
sense. The pattern-by-pattern translation, every MapStruct annotation shape next to its telescope equivalent, is the
[migration guide](docs/mapstruct-migration.md)'s table.
...
> **Convinced?** Add `implementation("io.github.eschizoid:telescope-core:1.3.0")`, write your next mapper as one
